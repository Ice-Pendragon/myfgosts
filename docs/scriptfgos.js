$(function () {
	$('#btn').click(function() {
		...
		//Check Class
		var $NonBeastClassCount = ($('select[name="Class"]')[0].children)[0].children.length + ($('select[name="Class"]')[0].children)[1].children.length;

		$ClassNumber = $('select[name="Class"]')[0].selectedIndex;
		if($ClassNumber < $NonBeastClassCount){
			$('#ClassOut').html($('select[name="Class"]')[0][$ClassNumber].innerHTML);
		}else{
			$('#ClassOut').html(($('select[name="Class"]')[0].children)[2].label);
		}
